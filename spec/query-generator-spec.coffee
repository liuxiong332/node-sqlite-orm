...
describe 'query generator', ->
  it 'columnDef', ->
    ...
    attr = {default: 'value', type: 'INT'}
    stmt = QueryGenerator.columnDef 'Name', attr
    stmt.should.equal "`Name` INT DEFAULT \"value\""

    attr = {references: {name: 'Table', fields: 'key'}, type: 'INT'}
    stmt = QueryGenerator.columnDef 'Name', attr
    stmt.should.equal "`Name` INT REFERENCES Table (`key`)"

    attr = {references: {name: 'TABLE', fields: ['id', 'name']}, type: 'INT'}
    stmt = QueryGenerator.columnDef 'Name', attr
    stmt.should.equal "`Name` INT REFERENCES TABLE (`id`, `name`)"
